...
contract('DaoBaseAuto', (accounts) => {
    ...
    it('should be able to add group member with AAC (direct call)', async() => {
        // set permissions 
        await daoBase.allowActionByAddress("manageGroups", aacInstance.address);

        // check permissions
        const isCanDoAction = await daoBase.isCanDoAction(creator, "manageGroups");
        assert.equal(isCanDoAction, true, 'Creator should have permission to run action');
        ...
        // Direct call
        await aacInstance.addGroupMemberAuto("Employees", employee1);
